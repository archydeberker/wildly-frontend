...
class Weather extends Component {
    constructor(props) {
        super()
        this.state = {
            id: props.id + props.type + "-div" + Math.ceil(Math.random() * 100).toString(),
            url: props.url,
            type: props.type,
        }
    }
    ...
    render() {
        return (
            <Grid
                ...
                flexGrow={1}
                alignSelf="center"
                style={{ alignSelf: "center", alignContents: "center" }}
                id={this.state.id}
            ></Grid>
        )
    }
    ...
    return (
        <div>
            {props.location ? (
                <Card>
                    <CardActionArea
                        onClick={() => props.handleClickOpen(props.location.fields.title)}
                        id={props.location.fields.title}
                    >
                        <CardMedia
                            style={{ paddingTop: "56.25%", width: "100%", height: "100%" }}
